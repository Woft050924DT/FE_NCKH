  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Get thesis tasks and build project timeline
        ...
        if (currentThesisId) {
          try {
            ...
            projectTimeline = (thesisTasks || []).map((task: any) => ({
              label: task.task_name,
              date: new Date(task.due_date).toLocaleDateString('vi-VN'),
              completed: task.status === 'COMPLETED',
              rawDate: new Date(task.due_date).getTime()
        ...
        try {
          const groups = await thesisGroupsService.getThesisGroups(user?.id);
          if (groups && groups.length > 0) {
            userIsLeader = groups[0].thesis_group_members?.find(
      ...
      <Card className="mt-6">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={() => navigate('/reports')}>Xem tất cả</Button>
          </div>
        </CardHeader>
        <CardContent>
